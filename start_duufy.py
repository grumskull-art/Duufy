...
def start_server():
    ...
    venv_python = r"C:\Users\Grums\Documents\App_projekt_python\.venv\Scripts\python.exe"
    ...
    if server_process.poll() is None:
        print(f"{Colors.GREEN}✓ FastAPI server running on http://127.0.0.1:8000{Colors.END}")
...
def main():
    ...
    if ngrok:
        print(f"{Colors.CYAN}Ngrok:{Colors.END}     http://127.0.0.1:4040 (for public URL)")
    ...
    try:
        ...
        while True:
            ...
            if server.poll() is not None:
                print(f"\n{Colors.RED}Server stopped unexpectedly!{Colors.END}")
                break
        ...
        print(f"\n{Colors.CYAN}👋 Duufy is offline. See you later!{Colors.END}\n")
