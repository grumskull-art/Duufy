...
test_cases = [
    ...
    ("jeg skal have øllebørd, ost, agurker, så skal jeg huske marmelade, riberhus mellemlagret ost", 
     ["Øllebrød", "Ost", "Agurk", "Marmelade"]),  # Riberhus might not match perfectly
...
for i, (text, expected_items) in enumerate(test_cases, 1):
    ...
    success = all(any(exp.lower() in found.lower() for found in found_items) for exp in expected_items)
