# HEY LOBS - KOMPLET PROJEKT DOKUMENTATION
*Genereret: 25. december 2025*

## PROJEKT OVERSIGT
- **Navn:** Hey Lobs Shopping List Management System
- **Type:** FastAPI backend med AI voice parsing
- **Total kode:** 2676 linjer Python (23 filer)
- **AI:** Anthropic Claude 3.5 Sonnet
- **Performance:** 16x speedup (7.6K  122K ops/s)
...
_CATEGORIES: dict[str, Category] = {
    'mælk': 'mejeri', 'letmælk': 'mejeri', 'minimælk': 'mejeri', 'sødmælk': 'mejeri',
    'smør': 'mejeri', 'ost': 'mejeri', 'fløde': 'mejeri', 'piskefløde': 'mejeri',
    'yoghurt': 'mejeri', 'skyr': 'mejeri', 'cremefraiche': 'mejeri', 'kærnemælk': 'mejeri',
    'ymer': 'mejeri', 'mozzarella': 'mejeri',
    'kylling': 'kød', 'oksekød': 'kød', 'hakket': 'kød', 'hakkekød': 'kød',
    'svinekød': 'kød', 'bacon': 'kød', 'pølser': 'kød', 'hamburgerryg': 'kød',
    'rullepølse': 'kød', 'leverpostej': 'kød', 'skinke': 'kød', 'medister': 'kød',
    'kalvekød': 'kød', 'lammekød': 'kød',
    'laks': 'fisk', 'tun': 'fisk', 'torsk': 'fisk', 'rejer': 'fisk',
    'brød': 'bager', 'rugbrød': 'bager', 'franskbrød': 'bager', 'boller': 'bager',
    'kartofler': 'grønt', 'kartoffel': 'grønt', 'løg': 'grønt', 'hvidløg': 'grønt',
    'gulerødder': 'grønt', 'gulerod': 'grønt', 'tomater': 'grønt', 'tomat': 'grønt',
...
_ALIASES: dict[str, str] = {
    'hambo': 'hamburgerryg', 'remu': 'remoulade', 'karto': 'kartofler',
    'toma': 'tomater', 'gule': 'gulerødder', 'sømælk': 'sødmælk',
    'smæølk': 'sødmælk', 'piskflø': 'piskefløde', 'rugbrø': 'rugbrød',
    'franskbrø': 'franskbrød', 'lever': 'leverpostej', 'rulle': 'rullepølse',
}
...
_FILLER_PATTERNS = tuple(map(lambda p: re.compile(p, re.IGNORECASE), [
    r"^(øh|ehm|øhm|nå|nåh|altså|ikke|jo|bare)\s+",
    r"^(jeg skal have|vi skal have|skal have|jeg skal|vi skal)\s+",
...
_HALF_UNIT_PATTERN = re.compile(
    r"^(en|et|halv|halvt)\s+(liter|l|kilo|kg)\s+(.+)$",
    re.IGNORECASE
)
...
def _normalize_unit(unit: str) -> str:
    ...
    u = unit.lower()
    return ('L' if u in ('l', 'liter') else
            'kg' if u in ('kg', 'kilo') else
            'g' if u in ('g', 'gram') else
            'ml' if u in ('ml', 'milliliter') else
            'dl' if u in ('dl', 'deciliter') else
            'stk' if u in ('stk', 'stykker', 'stykke') else
            'pk' if u in ('pk', 'pakke', 'pakker') else unit)
...
def local_parse(text: str) -> list[ParsedItem]:
    ...
    for part in parts:
        ...
        if match := _ORDTAL_UNIT_PATTERN.match(part):
            ...
            quantity = f"{_WORD_TO_NUM.get(num.lower(), num)} {_normalize_unit(unit)}"
...
Output JSON array: [{{"item":"Produkt","quantity":"1 L","category":"mejeri"}}]"""
    ...
    try:
        ...
        return [item for item in parsed if all(k in item for k in ('item', 'quantity', 'category'))]
    except Exception:
        return []
...
async def smart_parse_async(text: str, force_ai: bool = False) -> ParseResult:
    ...
    use_ai = force_ai or (not local or all_unknown or len(text.split()) < 4) and ANTHROPIC_AVAILABLE
...
from ai_parser import smart_parse, local_parse
...
`md
# Speech to Text Notification API

A FastAPI-based backend service that integrates speech recognition, Firebase Cloud Messaging, and Firestore database for real-time notifications.

## Features

- **User Management**: Register, retrieve, update, and delete users
- **Speech Recognition**: Transcribe audio files and URLs using Google Cloud Speech-to-Text or OpenAI Whisper
- **Firebase Notifications**: Send single, multicast, and topic-based notifications via Firebase Cloud Messaging
- **Firestore Database**: Store user data and speech recognition history
...
├── database.py            # Firestore connector
├── notifications.py       # Firebase Cloud Messaging handler
...
- Google Cloud credentials (for Speech-to-Text) or OpenAI API key (for Whisper)
...
4. **Configure environment variables**
   ...
   # Speech Recognition
   SPEECH_PROVIDER=google  # or 'whisper'
...
### Speech Recognition
- `POST /speech/transcribe` - Transcribe audio from URL
- `POST /speech/upload` - Upload and transcribe audio file
- `GET /users/{user_id}/speech-history` - Get user's transcription history

### Notifications
- `POST /notifications/send` - Send notification to user
- `POST /notifications/topic` - Send notification to topic
...
- Debug mode
- Speech provider (Google or Whisper)
- Server host and port

## Dependencies

See [requirements.txt](requirements.txt) for all dependencies. Key libraries:

- **FastAPI**: Modern web framework
- **Uvicorn**: ASGI server
- **Pydantic**: Data validation
- **Firebase Admin**: Firebase integration
- **Google Cloud Speech**: Speech-to-text service
...
- Speech recognition is async-compatible
- Long-running operations use background tasks
- Firestore operations are optimized with proper indexing
      ...
      "items": [
        {
          ...
          "quantity": "1 kg",
          "added_by": "Thore",
          "timestamp": "2025-12-25T18:48:15.841484"
        },
        {
          "name": "Kaffe",
          "quantity": "1 L",
          "added_by": "Thore",
          "timestamp": "2025-12-25T18:48:15.940616"
        },
        {
          "name": "Flø",
          "quantity": "1 stk",
          "added_by": "Thore",
          "timestamp": "2025-12-25T18:48:16.027412"
        },
        {
          "name": "Ost",
          "quantity": "400 g",
          "added_by": "Thore",
          "timestamp": "2025-12-25T18:48:16.126037"
        },
        {
          "name": "Så skal jeg have den der",
          "quantity": "1 stk",
          "added_by": "Thore",
          "timestamp": "2025-12-25T18:48:16.235438"
        }
      ],
      "created": "2025-12-25T18:47:58.786208",
      "last_updated": "2025-12-25T18:48:16.235445"
    }
  },
  "active_groups": [
    "hjemme"
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Accept invitation - Hey Lobs</title>
    <style>
        body { 
            font-family: Arial, sans-serif; 
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
        ...
        .group-name { color: #667eea; font-weight: bold; }
    </style>
</head>
<body>
    <div class="card">
        ...
        <div class="info">
            <p><strong>{{ inviter_name }}</strong> har inviteret dig til gruppen:</p>
            <p class="group-name">📋 {{ group_name }}</p>
        </div>
        <form id="acceptForm">
            <label for="username">Dit navn/kaldenavn:</label>
            <input type="text" id="username" name="username" placeholder="fx. Peter" required>
            <button type="submit">✓ Accepter invitation</button>
        </form>
    </div>
    ...
    <script>
        document.getElementById('acceptForm').addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('/invite/{{ token }}/accept', {
                    method: 'POST',
